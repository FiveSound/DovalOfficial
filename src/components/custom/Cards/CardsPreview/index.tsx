...
const CardsPreview = (props: Props) => {
  ...
  return (
    <FlexContainer newStyle={styles.container} key={row.id}>
        <FlexContainer variant="row" newStyle={[styles.flexRow, { backgroundColor: backgroundMaingrey }]}>
          { showAvatar && <Avatars size="large" source={`${CLOUDFRONT}${user?.avatar}`} />}
          {!showAvatar && (custom) }
          <FlexContainer>
            <Typography variant="H4title" numberOfLines={1} newStyle={styles.typography}>
              {row.label}
            </Typography>
            <Typography variant="SubDescription" numberOfLines={2} newStyle={styles.description}>
              {row.description}
            </Typography>
          </FlexContainer>
        </FlexContainer>

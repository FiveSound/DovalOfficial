...
const CardGallery = React.memo(
  ({
    ...
    return (
      <Pressable key={mediaKey} onPress={onPressImg} style={styles.container}>
        <FlexContainer
          newStyle={
        ...
        >
          {isSelected ? (
            <Typography variant="H4title" newStyle={styles.selectedText}>
              {CountSelect}
            </Typography>
          ) : (
            <View />
        ...
        <FlexContainer newStyle={styles.durationContainer}>
          <Typography variant="H4title">{duration}</Typography>
        </FlexContainer>
      </Pressable>
    );
  },

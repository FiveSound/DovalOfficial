...
import { getFollowersAccountsService } from '../services/follows';
...
export function UseFollowers({
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const result = await getFollowersAccountsService(userID, user, state.page);
            if (result) {
                dispatch({ type: 'SET_DATA', payload: result });
            } else {
                throw new Error('No data returned from getFollowersAccountsService');
            }
        } catch (err) {
    ...
    }, [userID, user, state.page]);

    useEffect(() => {
        fetchData();
    }, [fetchData, state.page, userID, user]);
    ...
    useEffect(() => {
        if (Array.isArray(state.data)) {
            ...
            dispatch({
                ...
                payload: state.page === 1 ? filtered : [...state.filteredData, ...filtered],
            });
            setTimeout(() => {

...
export const useMediaLibrary = () => {
  ...
  const fetchAlbums = useCallback(async () => {
    ...
    try {
      ...
      const allMediaAlbum = {
        id: 'all-media',
        title: 'Recientes',
        assetCount: formattedAssetCount,
        uriAsset: allAssets.assets[0]?.uri,
      };

      const albumsWithUri = await Promise.all(
  ...
  const fetchMedia = useCallback(
    async (count: number, albumId?: string) => {
      // setIsLoading(true);
          ...
          assets.assets.map(async (asset, index) => {
            ...
            return {
              ...
              node: {
                ...
                uri: uri,
                duration:
                  asset.mediaType === 'video' ? asset.duration : undefined,

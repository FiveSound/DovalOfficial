...
export const acceptOrderService = async (orderID: number, delivery: boolean) => {
  try {
    const userToken = await AsyncStorage.getItem("userToken");
...
export const getRecipesByBusinessIDService = async ({ queryKey }: QueryKeyType) => {
  try {
...
export const getDetailsBusinessIDService = async (location: object, businessID: string) => {
  try {
...
export const getNearbyRecipesService = async (location: object) => {
  try {
    const response = await axios.post(`${API_URL}/api/business/recipes/nearby`, {
      ...location,
    });
    return response.data;
  } catch (error) {

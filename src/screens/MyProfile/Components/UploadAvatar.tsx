  ...
  try {
    ...
    if (!result.canceled && result.assets) {
      ...
      for (let index = 0; index < assets.length; index++) {
        ...
        const fileExtension = manipResult.uri.substr(manipResult.uri.lastIndexOf(".") + 1);
        ...
        if (response && response.key) {
          ...
          console.error('La subida de la imagen falló o la respuesta no contiene la clave esperada.');
      ...
      console.log("Selección de imagen cancelada o no se seleccionaron assets.");
    }
  } catch (error) {

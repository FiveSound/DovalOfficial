...
const SideDishSelector: React.FC<Props> = ({ variants, navigation }) => {
  ...
  return (
    ...
    AppendPreview={
      <Perks label={variants.data?.resume.length === 0 ? 'Required' : 'Completed'} status={variants.data?.resume.length === 0 ? 'error' : 'success'} Reverse={false}/>
    }
    append={
      variants.isLoading ? (
        <IsLoading />
        ...
        contentContainerStyle={styles.scrollView}>
          <FlexContainer variant='row' newStyle={styles.flexContainer}>
            {variants.data?.resume.map((row: any) => (
              <Icons 
                key={row.id}
                appendIcons={<Typography variant='H4title' newStyle={styles.text}>Varianst: {row.title}</Typography>} 
                styles={styles.icon}
              />
            ))}
          </FlexContainer>

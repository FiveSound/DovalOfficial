...
const Video = React.memo(({ postID, uri, height, onError, onLoad, isItemFocused }: VideoProps) => {
  useAudioConfig();
  const { videoRef, isPlaying, reloadVideo, togglePlayPause } = useVideoControl(uri, isItemFocused);
  ...
  return (
    <Pressable >
      ...
      <View style={styles.containerReproduccion}>
        <ProgressBar progress={progress} duration={duration} videoRef={videoRef} />
      </View>
      {isManuallyPaused && isPaused && <Pause isPaused={isPaused} handlePause={handleTogglePlayPause} />}
    </Pressable>
  );
});

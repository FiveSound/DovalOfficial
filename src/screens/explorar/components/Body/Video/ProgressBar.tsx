...
const ProgressBar = ({ progress, duration, videoRef }: any) => {
  ...
  const handlePanResponderGrant = useCallback((evt) => {
  ...
  const handlePanResponderMove = useCallback((evt) => {
    const screenWidth = Dimensions.get("window").width;
    const seekPosition = (evt.nativeEvent.locationX / screenWidth) * duration;
    if (videoRef.current && seekPosition >= 0 && seekPosition <= duration) {
  ...
  const handlePanResponderRelease = useCallback((evt) => {
    const screenWidth = Dimensions.get("window").width;
    const seekPosition = (evt.nativeEvent.locationX / screenWidth) * duration;
    if (videoRef.current && seekPosition >= 0 && seekPosition <= duration) {
      const positionMillis = seekPosition * 1000;
  ...
  return (
    ...
    >
      <Animated.View
        style={[styles.progressBar, animatedProgressBarStyle]}
      />
    </View>
  );
};

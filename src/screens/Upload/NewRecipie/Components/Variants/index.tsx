...
const Variants = () => {
  ...
  const handleAddVariant = async () => {
    ...
    queryClient.setQueryData(["recipe-variants-component", values.id], ({ variants, ...rest }: TypeData) => ({
      ...rest,
  ...
  const handleAddSubVariant = async (id: number) => {
    ...
    queryClient.setQueryData(["recipe-variants-component", values.id], (oldData: any) => ({
      ...oldData,
  ...
  if (data) {
    return (
      <Container label={i18next.t("Side Dish Everyone")} showBack={true} showHeader={true}>
        <ScrollView>
          <View style={styles.container}>
            <Hero
              label={i18next.t("Side Dish")}
              sublabel={i18next.t("Add variants to your recipe, and help your users understand what they want to serve it with.")}
            />
            ...
            {data.variants.map((row: Variant) => (
              <VariantComponent
                ...
                subvariants={data.subvariants.filter((item: SubVariant) => item.variantID === row.id)}
                required={Boolean(row.required)}
              />
            ))}

            <VariantComponent
              title={i18next.t("Add title variant")}

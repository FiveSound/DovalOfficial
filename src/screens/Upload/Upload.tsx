...
const Upload = () => {
  ...
  const { media, fetchMedia, albums, fetchAlbums, fetchMoreMedia } =
    useMediaLibrary();
  ...
  const [selectionOrder, setSelectionOrder] = useState<Map<string, number>>(
    new Map(),
  );
  ...
  const toggleSelection = useCallback(
    (mediaId: string, mediaType: string, mediaUri: string) => {
        ...
        if (newSelectedIds.has(mediaId)) {
          ...
          if (mediaType === 'photo') {
            if (pickedMedia.some(media => media.type === 'video')) {
              Alert.alert(
                i18next.t(
                  'Selection not allowed You cannot select images because you already have a video selected.',
                ),
              );
            ...
            if (newSelectedIds.size < 10) {
              ...
              setPickedMedia([
                ...pickedMedia,
                { id: mediaId, type: mediaType, uri: mediaUri },
              ]);
              setSelectionOrder(prevOrder => {
                const newOrder = new Map(prevOrder);
                const nextIndex =
                  Array.from(newOrder.values()).reduce(
            ...
            if (pickedMedia.some(media => media.type === 'photo')) {
              Alert.alert(
                i18next.t(
                  'Selection not allowed, You cannot select a video because you already have images selected.',
                ),
              );
              return newSelectedIds;
            }
      ...
      return (
        <Suspense fallback={<SkeletonAssets />}>
          <LazyCard
            ...
            mediaType={mediaType}
            onPressImg={() =>
              toggleSelection(item.node.id, item.node.mediaType, mediaUri)
            }
  ...
  if (!hasPermission) {
    return (
      <ScreenEmpty
        ...
        subLabel={i18next.t(
          'You need to give us permission to access this area',
        )}
        ShowButton={true}

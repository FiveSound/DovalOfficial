...
const Recipes = memo(() => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((recipe: any) => recipe.name.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [data, searchTerm]);
  ...
  return (
    <Container showHeader={true} label={i18next.t("My MenuRecipes")}>
      <SearchHeader onChange={setSearchTerm} placeholder={i18next.t("Search")} value={searchTerm} showBack={false} />
      <FlatList
        data={filteredData}
        renderItem={({ item }) => {

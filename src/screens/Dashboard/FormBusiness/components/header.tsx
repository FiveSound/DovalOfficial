...
type Props = {
  ...
  onPress?: () => void
  loading?: boolean
  variant?: "primary" | "secondary" | "disabled" | "transparent" | "outline" | "error"
  labelButtons: string
  showDivider?: boolean
...
const Header = (props: Props) => {
  const { label, disabled, onPress, loading, variant, labelButtons, showDivider = true, showBackPersonal = true, onPressBack } = props
  ...
  return (
    <>
      <SafeAreaView style={styles.header}>
        {
          showBackPersonal ? <></> :
            <TouchableOpacity onPress={onPressBack} style={[styles.backPersonal, {backgroundColor: backgroundMaingrey}]}>
             <Typography variant='H4title'> Back</Typography>
            </TouchableOpacity>
        }
        <Typography variant='subtitle' testID='header-label'>{label}</Typography>
        <Buttons
          label={labelButtons}
          disabled={disabled}
          onPress={onPress}

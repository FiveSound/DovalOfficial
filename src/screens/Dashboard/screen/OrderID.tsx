     ...
     <Container style={styles.container}>
       <ScrollView>
        <View style={styles.container}>
          <View style={[styles.group, { alignItems: "center", marginBottom: 10 }]}>
            <Text style={{ fontSize: 30, fontWeight: "bold", color: "#222222" }}>Orden no.</Text>
            <Text style={{ fontSize: 30, fontWeight: "bold", color: "#FF5500" }}>#{order.data.id}</Text>
          ...
          <View style={styles.box}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <Product title={item.name} cover={item.cover} variants={item.variants} quantity={item.quantity} />
              )}
              keyExtractor={(item) => item.id.toString()}
            />

            <View style={styles.group}>
              <TouchableOpacity style={[styles.button, { borderColor: "#4ADE80" }]}>
                <Text style={[styles.textButton, { color: "#4ADE80" }]}>Aceptar orden</Text>
              </TouchableOpacity>

              <TouchableOpacity style={[styles.button, { backgroundColor: "#F41F52", borderColor: "#F41F52" }]}>
                <Text style={[styles.textButton, { color: "#FFF" }]}>Rechazar orden</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.box}>
            <View style={{ marginBottom: 15 }}>
              ...
              <Text style={styles.text}>{order.data.rider ? order.data.rider : "No disponible"}</Text>
              <Text style={styles.text}>{order.data.riderOrders} Ordenes completadas</Text>
          ...
          <View style={styles.box}>
            <Text style={[styles.title, { marginBottom: 10 }]}>Informacion de pago:</Text>

            <View style={[styles.group, { justifyContent: "space-between" }]}>
              <Text style={[styles.text]}>Items</Text>
              <Text style={[styles.text]}>{order.data.resume.items_costs}</Text>
            </View>
            ...
            <View style={[styles.group, { justifyContent: "space-between" }]}>
              ...
              <Text style={[styles.text]}>{order.data.resume.delivery_costs}</Text>
            </View>

            <View style={[styles.group, { justifyContent: "space-between" }]}>
              <Text style={[styles.text]}>Service</Text>
              <Text style={[styles.text]}>{order.data.resume.service_costs}</Text>
            </View>
            ...
            <View style={[styles.group, { marginTop: 10, justifyContent: "space-between" }]}>
              <Text style={[styles.text, { fontSize: 18 }]}>Pagado por el cliente</Text>
              <Text style={[styles.text, { fontSize: 20, fontWeight: "bold" }]}>{order.data.resume.total}</Text>
            </View>
          </View>
